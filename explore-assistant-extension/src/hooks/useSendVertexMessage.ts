...
const useSendVertexMessage = () => {
  ...
  const modelName = lookerHostData?.extensionId.split('::')[1]
  ...
  const vertexBigQuery = async (
    ...
    try {     // Escape special characters
      const sanitizedContents = `"${contents
        ...
        .replace(/\t/g, ' ') 
        .replace(/,/g, ' ')  }"`
      const { id } = await core40SDK.ok(core40SDK.create_query(
        {
      ...
      console.log(query)
      const exploreData = await query[0]['explore_assistant.generated_content']
...
${exploreRefinementExamples &&
  ...
  const generateSharedContext = (dimensions: any[], measures: any[], exploreGenerationExamples: any[], filters: boolean = false) => {
